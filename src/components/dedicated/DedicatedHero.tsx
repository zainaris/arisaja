      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-32 w-full">
        <div className="max-w-2xl space-y-8">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight text-white drop-shadow-lg">
            {t("ded.hero.title1")}{" "}<span className="text-gradient-green">{t("ded.hero.title2")}</span>
          </h1>
          <p className="text-lg text-white/90 max-w-xl leading-relaxed drop-shadow-md">{t("ded.hero.desc")}</p>
          <div className="flex flex-wrap gap-4">
            <a href="#pricing" className="px-8 py-4 rounded-2xl bg-primary text-primary-foreground font-semibold text-lg hover:bg-primary-dark transition-all duration-300 shadow-lg hover:shadow-xl">{t("ded.hero.btn1")}</a>
            <a href={WA_LINK} target="_blank" rel="noopener noreferrer" className="px-8 py-4 rounded-2xl border-2 border-white/40 text-white font-semibold text-lg hover:bg-white/10 transition-all duration-300 backdrop-blur-sm">{t("common.wa")}</a>
          </div>
