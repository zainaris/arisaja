    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-12">
        ...
        <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
          Terhubung ke 10 datacenter utama di area Jabodetabek & Cikarang melalui jaringan DWDM berkapasitas tinggi.
        </p>
      </div>
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-2xl mb-12 border border-primary/20">
        <img
          src={datacenterMap}
          alt="Peta lokasi datacenter DWDM Artamedia"
        ...
        <p className="absolute bottom-3 left-1/2 -translate-x-1/2 text-foreground/60 text-sm font-medium tracking-wide">
          Jabodetabek & Cikarang DWDM Datacenter Coverage
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {datacenters.map((dc) => (
          <div
            key={dc.id}
            className="group relative bg-card border border-primary/10 rounded-xl p-4 hover:border-primary/30 hover:shadow-card-hover transition-all duration-300"
          >
            <div className="flex items-start gap-3">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm group-hover:bg-primary/20 transition-colors">
                {dc.id}
              </div>
              <div className="min-w-0">
                <h4 className="text-card-foreground font-semibold text-sm leading-tight">
                  {dc.name}
                </h4>
                <div className="flex items-center gap-1 mt-1">
                  <MapPin className="w-3 h-3 text-primary/60" />
                  <span className="text-muted-foreground text-xs">{dc.area}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="text-center mt-10">
        <a
          href="https://wa.me/6282280257572?text=Halo%2C%20saya%20ingin%20mengetahui%20jangkauan%20DWDM%20di%20area%20saya"
          target="_blank"
